...
namespace GuestService.Code.Payment
{
    public class PayUPaymentResult
    {
        ...
        public PayUPaymentResult.OpeationStatus Status
        {
            get;
            private set;
        }
        public static PayUPaymentResult Create(HttpRequestBase request)
        {
            if (request == null)
            {
            ...
            return new PayUPaymentResult
            {
                InvoiceNumber = invoiceNumber,
                Status = (status.ToLower() == "authorized" || status.ToLower() == "paid") ? PayUPaymentResult.OpeationStatus.Success : PayUPaymentResult.OpeationStatus.Failed
            };
        }
    }
}
